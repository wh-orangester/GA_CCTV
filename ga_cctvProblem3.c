...
void evaluateCCTVNumberFitness(GENE_T * individual) {
    ...
    for(i = 0; i < c_count; i++) {
        ...
        // Find total CCTV used
        cctv_used = 0;
        for(i = 0; i < c_count; i++) {
            if((*individual).type[i] != 0) cctv_used++;
        }
        // Calculate base fitness
        (*individual).fitness = 0.5 + (c_count - cctv_used) / (2.0 * c_count);
        // Apply penalty if some artifact is not monitored
        p_count = 0;
        for(j = 0; j < a_count; j++) {
            monitored = 0;
            for(i = 0; i < c_count && !monitored; i++) {
                monitored = (monitored && visible[i][j]);
            }
            if(!monitor) p_count++;
        }
        // If penalty should be applied
        if(p_count > 0) {
            penalty_ratio = BASE_UNMONITORED_PENALTY + (1 - BASE_UNMONITORED_PENALTY) * (p_count - 1.0) / (c_count - 1.0);
...
GENE_T * initialize() {
    ...
    for(i = 0; i < POPULATION_SIZE; i++)
        evaluateCCTVNumberFitness(&population[i]);
...
GENE_T * evolve(GENE_T * population) {
    ...
    for (i=eliteMembers;i<POPULATION_SIZE;i++){
        ...
        evaluateCCTVNumberFitness(&evoPop[i]);
    }
